...
import { RegressionBaselineComponent } from '../components/common/regression-baseline.component';

@Component({
  standalone: true,
  imports: [DatePipe, DecimalPipe, SlicePipe, FormsModule, MatButtonModule, MatFormFieldModule, MatInputModule, MatProgressBarModule, MatSelectModule, LucideAngularModule, CellStateBadgeComponent, SafetyCanvasComponent, FindingDrawerComponent, RegressionBaselineComponent],
    ...
    <section class="validation-grid">
      <div class="run-register">
        ...
        @for (run of runs.items(); track run.id) {
          <button type="button" class="run-row" [class.selected]="runs.selected()?.id === run.id" (click)="runs.choose(run)">
            <span class="run-id">#{{ run.id }}</span><span><strong>{{ run.program_code }} · v{{ run.program_version }}</strong><small>{{ run.started_at | date:'MMM d, HH:mm:ss' }} · attempt {{ run.attempt }}@if (run.baseline_run_id) { · baseline #{{ run.baseline_run_id }}@if (run.regression?.has_new_findings) { · NEW FINDINGS } }</small></span><span class="risk">{{ run.risk_score | number:'1.0-0' }}</span><app-cell-state-badge [state]="run.validation_status" />
          </button>
      ...
      <article class="evidence-panel">
        @if (runs.selected(); as run) {
          ...
          <app-regression-baseline [regression]="run.regression ?? null" />
          <section class="review-block">
            ...
            @if (canReview() && run.validation_status === 'reviewed') {
              @if (run.regression; as regression) {
                @if (regression.has_new_findings) {
                  <p class="accept-blocked"><lucide-icon name="octagon-x" [size]="15" />Acceptance is blocked: {{ regression.collision_diff.added.length }} new envelope and {{ regression.interlock_diff.added.length }} new interlock finding(s) versus accepted baseline #{{ regression.bound.id }}. Resolve the regressions or establish a new accepted baseline first.</p>
                }
              }
              <mat-form-field appearance="outline"><mat-label>Acceptance note</mat-label><input matInput [(ngModel)]="reviewNote" /></mat-form-field><button mat-flat-button color="primary" type="button" (click)="accept(run)" [disabled]="reviewNote.trim().length < 8 || !!run.regression?.has_new_findings"><lucide-icon name="circle-check" [size]="16" />Accept evidence</button><button mat-stroked-button type="button" (click)="voidRun(run)" [disabled]="reviewNote.trim().length < 8">Void</button>
            }
