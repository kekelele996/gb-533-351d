...
import { RegressionDiffComponent } from '../components/common/regression-diff.component';

@Component({
  standalone: true,
  imports: [DatePipe, DecimalPipe, SlicePipe, FormsModule, MatButtonModule, MatFormFieldModule, MatInputModule, MatProgressBarModule, MatSelectModule, LucideAngularModule, CellStateBadgeComponent, SafetyCanvasComponent, FindingDrawerComponent, RegressionDiffComponent],
    ...
    <section class="validation-grid">
      <div class="run-register">
        ...
        @for (run of runs.items(); track run.id) {
          <button type="button" class="run-row" [class.selected]="runs.selected()?.id === run.id" (click)="runs.choose(run)">
            <span class="run-id">#{{ run.id }}</span><span><strong>{{ run.program_code }} · v{{ run.program_version }}</strong><small>{{ run.started_at | date:'MMM d, HH:mm:ss' }} · attempt {{ run.attempt }} · @if (run.baseline) { baseline #{{ run.baseline.id }} } @else { no baseline }</small></span><span class="risk">{{ run.risk_score | number:'1.0-0' }}</span><span class="row-tail">@if (run.baseline && run.regression_diff.has_new_findings && run.validation_status !== 'voided') { <lucide-icon name="octagon-alert" [size]="15" class="regression-flag" /> }<app-cell-state-badge [state]="run.validation_status" /></span>
          </button>
      ...
      <article class="evidence-panel">
        @if (runs.selected(); as run) {
          ...
          <app-regression-diff [diff]="run.regression_diff" [baseline]="run.baseline" />
          <section class="review-block">
            ...
            @if (canReview() && run.validation_status === 'reviewed' && run.baseline && run.regression_diff.has_new_findings) { <p class="accept-blocked"><lucide-icon name="octagon-alert" [size]="15" />Acceptance locked: {{ run.regression_diff.new_collision_count }} new collision and {{ run.regression_diff.new_interlock_count }} new interlock finding(s) are absent from the bound baseline.</p> }
            @if (canReview() && run.validation_status === 'reviewed') { <mat-form-field appearance="outline"><mat-label>Acceptance note</mat-label><input matInput [(ngModel)]="reviewNote" /></mat-form-field><button mat-flat-button color="primary" type="button" (click)="accept(run)" [disabled]="reviewNote.trim().length < 8 || (!!run.baseline && run.regression_diff.has_new_findings)"><lucide-icon name="circle-check" [size]="16" />Accept evidence</button><button mat-stroked-button type="button" (click)="voidRun(run)" [disabled]="reviewNote.trim().length < 8">Void</button> }
